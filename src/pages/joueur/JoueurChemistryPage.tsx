...
export function JoueurChemistryPage() {
  ...
  const { chemistry, squadPlayers } = useJoueurBackendData();

  // Build node list from squad players (use up to 6)
  const playerNodes = squadPlayers.slice(0, 6);

  // Filter chemistry pairs to only those with both players in nodes
  const nodeIds = new Set(playerNodes.map((p) => p.id));
  const visiblePairs = chemistry.filter(
    (c) => nodeIds.has(c.player1Id) && nodeIds.has(c.player2Id),
  );
  ...
  return (
    <motion.div className="space-y-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }}>
      <GlassCard raised className="p-5">
        ...
        <p className="mb-6 text-xs" style={{ color: "var(--text-muted)" }}>Compatibilité entre joueurs — {squadPlayers.length} joueurs dans l'effectif</p>
        ...
        {playerNodes.length >= 2 ? (
            ...
            {playerNodes.map((player, i) => {
              ...
              return (
                <motion.button
                  key={player.id}
                  type="button"
                  onClick={() => navigate(`/joueurs/comparer`)}
          ...
          <div className="flex h-64 items-center justify-center text-sm" style={{ color: "var(--text-muted)" }}>
            Pas assez de joueurs pour afficher le réseau
          </div>
        )}
      </GlassCard>

      {/* Chemistry pairs table */}
      {chemistry.length > 0 ? (
              ...
              <GlassCard className="flex items-center gap-4 p-4">
                <div className="flex-1 text-center">
                  <p className="text-xs font-semibold" style={{ color: "var(--text-primary)" }}>
                    {pair.player1Name.split(" ").pop()}
                  </p>
                </div>
                <ChemistryGauge value={pair.chemistry} />
                <div className="flex-1 text-center">
                  <p className="text-xs font-semibold" style={{ color: "var(--text-primary)" }}>
                    {pair.player2Name.split(" ").pop()}
                  </p>
                </div>
              </GlassCard>
            </motion.div>
        ...
        <GlassCard raised className="p-8 text-center">
          <p className="text-sm" style={{ color: "var(--text-muted)" }}>Aucune donnée de chimie disponible</p>
        </GlassCard>
      )}

      <GlassCard className="p-4">
        <p className="text-xs" style={{ color: "var(--text-muted)" }}>
          {squadPlayers.length} joueurs dans l'effectif — {chemistry.length} paires analysées
        </p>
      </GlassCard>
    </motion.div>
  );
